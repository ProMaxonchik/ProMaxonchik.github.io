...
class GameCoordinator {
  ...
  deathSequence() {
    ...
    new Timer(() => {
        ...
        if (this.lives > 0) {
            ...
            const pointsToAdd = (this.points / 25) * 2;

            // Send the calculated score to the clicker game using localStorage or postMessage
            localStorage.setItem('pacmanScore', pointsToAdd);
            localStorage.setItem('openSpirinmanModal', 'true');
            // Redirect to the main page after the game over
            setTimeout(function() {
  ...
  dotEaten() {
    ...
    if (this.remainingDots === 0) {
        // Send the score to the clicker game before redirecting
        const pointsToAdd = (this.points / 25) * 2;

        // Send the calculated score to the clicker game using localStorage or postMessage
        localStorage.setItem('pacmanScore', pointsToAdd);
        localStorage.setItem('openSpirinmanModal', 'true');
        // Trigger win condition and redirect
        setTimeout(function() {
            window.location.href = '../../tasks.html';
